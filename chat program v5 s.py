...
def prs_msg(atext):
    T=input('enter msg type (file="F",text=blank): ')
    while T!='F' and T!='':
        print('Enter "F" or blank (file or text)')
        T=input('enter msg type (file="F",text=blank): ')
    msg=b'BLANK'
    if T=='':
            ...
            try:
                ...
                for i in range(len(route)):
                    if route[len(route)-1-i]=='/':
                        start=len(route)-i
                        break
                #to make sure that fn and ft are empty before processed (it is in a loop)
                fn=''
                ft=''
...
while True:
    ...
    print('To send to all people, enter nothing (blank)')
    mode=input('enter mode: ')
    while mode!='/getnames' and mode!='/spec' and mode!='':
        print('\nchoose a type')
    ...
    if mode=='/getnames':
        ...
        while True:
            target=input('Enter account (name) to send to (enter nothing or "/end" to finish): ')
            if target=='':
                target='/end'
